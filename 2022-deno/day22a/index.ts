...
function dirSymbol(d: Dir): string {
  switch (d) {
    case Dir.N:
      return "^";
    case Dir.S:
...
function drawMap(map: Map, states: State[]): string {
  const pixels: string[][] = [];
  for (let i = 0; i < map.rows.length; i++) {
    const rowPixels: string[] = [];
    pixels.push(rowPixels);

    const [start, end] = map.rows[i];
    for (let j = 0; j < map.cols.length; j++) {
      if (map.walls.has(vec2.key([i, j]))) {
        rowPixels.push("#");
        continue;
      }
  ...
  for (const [[row, col], dir] of states) {
    // if (
    //   pixels[row][col] == " " || pixels[row][col] == "#" ||
    //   pixels[row][col] == undefined
    // ) {
    //   throw new Error(
    //     `invalid step ${[[row, col], dir]} for pos ${[row, col]}: ${
    //       pixels[row][col]
    //     }`,
    //   );
    // }

    pixels[row][col] = dirSymbol(dir);
  }
