...
let find_sleepiest_guard guard_states =
  let guard, _ =
    Hashtbl.fold
      (fun guard states sleepiest ->
        let _, max = sleepiest in
        let total_asleep =
          List.fold_left
            (fun tot state_by_min -> tot + count_asleep state_by_min)
            0 states
        in
        if total_asleep > max then (guard, total_asleep) else sleepiest )
      guard_states (-1, -1)
  in
  guard
...
let find_sleepiest_min states =
  ...
  let best_min, _ =
    Array.fold_lefti
      (fun sleepiest min asleep ->
        let _, best_asleep = sleepiest in
        if asleep > best_asleep then (min, asleep) else sleepiest )
      (-1, -1) asleep_by_min
  in
  best_min
...
|}
  ...
  let sleepiest_guard = find_sleepiest_guard guard_states in
  let sleepiest_min =
    find_sleepiest_min (Hashtbl.find guard_states sleepiest_guard)
  in
  sleepiest_guard * sleepiest_min |> dump |> print_endline
