...
let () =
  let groups_by_id = parse data in
  let groups_by_id = run_battle groups_by_id in
  let remaining_groups = Hashtbl.values groups_by_id |> List.of_enum in
  remaining_groups |> groups_to_string |> print_endline ;
  Printf.printf "\nRemaining units: %d\n"
    (List.fold_left (fun accum g -> accum + g.nunits) 0 remaining_groups)
