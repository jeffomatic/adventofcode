...
let rec value root =
  match List.length root.children with
  | 0 -> List.sum root.metadata
  | _ ->
      List.fold_left
        (fun tot child_index ->
          let child_val =
            if child_index > List.length root.children then 0
            else value (List.nth root.children (child_index - 1))
          in
          tot + child_val )
        0 root.metadata
...
let () = parse data |> value |> dump |> print_endline
