...
let rooms_at_min_distance map min_distance =
  let rec aux next visited steps =
    if steps = min_distance then Hashtbl.length map - Set.cardinal visited
    else
      let next, visited =
        Set.fold
          (fun pos accum ->
...
let () =
  ...
  Printf.printf "Rooms at %d steps or more from origin: %d\n" 1000
    (rooms_at_min_distance map 1000)
