...
let follow m =
  ...
  let rec aux i j d steps =
    let next_i, next_j = advance i j d in
    if outside next_i next_j h w then steps
    else
      match (m.(next_i)).(next_j) with
      | Blank -> steps
      | Vertical | Horizontal -> aux next_i next_j d (steps + 1)
      | Letter c -> aux next_i next_j d (steps + 1)
      | Junction ->
          let d =
