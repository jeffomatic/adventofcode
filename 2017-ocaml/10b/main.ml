...
let read_input () = IO.stdin |> IO.read_all |> String.trim

let hex_encode ints =
  ints |> List.map (fun v -> Printf.sprintf "%x" v) |> String.concat ""

let () =
  read_input () |> String.to_list |> List.map Char.code |> knot_hash
  |> hex_encode |> print_endline
